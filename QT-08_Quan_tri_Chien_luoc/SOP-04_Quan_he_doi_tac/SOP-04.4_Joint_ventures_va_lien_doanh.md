...
```mermaid
flowchart TD
    A[Xác định<br/>opportunity] --> B[Đánh giá<br/>strategic fit]
    B --> C[Analyze<br/>alternatives]
    C --> D{JV là best<br/>option?}
    D -->|Yes| E[Proceed]
    D -->|No| F[Other options]
```
...
```mermaid
flowchart LR
    A[Trigger<br/>event] --> B[Notify<br/>partner]
    B --> C[Negotiate<br/>terms]
    C --> D[Valuation]
    D --> E[Execute<br/>exit]
    E --> F[Transition]
```
