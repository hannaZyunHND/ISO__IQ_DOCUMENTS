...
```mermaid
flowchart LR
    A[Review<br/>current coverage] --> B[Assess<br/>changes]
    B --> C[Update<br/>requirements]
    C --> D[Market<br/>comparison]
    D --> E[Negotiate]
    E --> F[Renew/Change]
```
...
```mermaid
flowchart TD
    A[Incident occurs] --> B[Immediate<br/>notification]
    B --> C[Document<br/>evidence]
    C --> D[File claim<br/>with insurer]
    D --> E[Insurer<br/>assessment]
    E --> F[Settlement]
```
