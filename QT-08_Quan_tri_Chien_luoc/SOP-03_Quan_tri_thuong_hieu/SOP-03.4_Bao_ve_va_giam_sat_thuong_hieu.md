...
```mermaid
flowchart LR
    A[Kiểm tra<br/>khả năng đăng ký] --> B[Chuẩn bị<br/>hồ sơ]
    B --> C[Nộp đơn<br/>Cục SHTT]
    C --> D[Thẩm định<br/>hình thức]
    D --> E[Công bố<br/>đơn]
    E --> F[Thẩm định<br/>nội dung]
    F --> G[Cấp văn bằng<br/>bảo hộ]
```
...
```mermaid
flowchart LR
    A[Monitor<br/>mentions] --> B[Analyze<br/>sentiment]
    B --> C{Negative?}
    C -->|Yes| D[Respond]
    C -->|No| E[Engage]
    D --> F[Track]
...
```mermaid
flowchart TD
    A[Phát hiện<br/>khủng hoảng] --> B[Activate<br/>crisis team]
    B --> C[Assess<br/>situation]
    C --> D[Develop<br/>response]
    D --> E[Execute<br/>communication]
    E --> F[Monitor<br/>reaction]
    F --> G{Resolved?}
    G -->|No| D
    G -->|Yes| H[Recovery]
```
