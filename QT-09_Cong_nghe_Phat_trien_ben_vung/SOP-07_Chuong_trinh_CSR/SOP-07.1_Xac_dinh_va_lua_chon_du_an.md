...
```mermaid
flowchart LR
    A[Identify<br/>issue] --> B[Research<br/>context]
    B --> C[Consult<br/>stakeholders]
    C --> D[Assess<br/>needs]
    D --> E[Prioritize]
    E --> F[Feasibility]
```
...
```mermaid
flowchart TD
    A[Submit<br/>proposal] --> B[CSR Team<br/>review]
    B --> C{Initial<br/>screen}
    C -->|Pass| D[Committee<br/>evaluation]
    C -->|Fail| E[Feedback/<br/>Decline]
    D --> F{Score ≥ 3.5?}
    F -->|Yes| G[BGH<br/>approval]
    F -->|No| E
    G --> H{Budget > 100M?}
    H -->|Yes| I[Board<br/>approval]
    H -->|No| J[Approved]
    I --> J
```
