...
```mermaid
graph TD
    A[Phát sinh nhu cầu sửa] --> B{Nguồn?}
    
    B -->|PH yêu cầu| C[PH gửi: Sổ/Email/Trực tiếp]
    B -->|Trường phát hiện| D[GVCN/NV phát hiện sai]
    
    C --> E{Loại TT?}
    D --> E
    
    E -->|Tĩnh: Tên, Ngày sinh...| F[NV lập Phiếu QT06-F44]
    F --> G[Kiểm tra giấy tờ]
    G --> H[Trình BP HS duyệt]
    H --> I{Duyệt?}
    I -->|Không| J[Từ chối, Giải thích PH]
    I -->|Có| K[NV thực hiện]
    K --> L[Scan giấy mới, Sửa Phần mềm, In lại Thẻ...]
    
    E -->|Động chậm: SĐT, Địa chỉ...| M[GVCN sửa ngay]
    M --> N[Test: Gọi thử SĐT mới]
    N --> O{Đúng?}
    O -->|Sai| P[Hỏi lại PH]
    O -->|Đúng| Q[✅ Hoàn thành]
    
    E -->|Động nhanh: Lớp, SK...| R[Cập nhật theo lịch]
    R --> S[Lên lớp: Tháng 6 - Bulk update]
    R --> T[Khám SK: Tháng 9 - Y tế cập nhật]
    
    E -->|Liên tục: Điểm, Nhận xét...| U[Cập nhật ngay khi có]
    U --> V[Điểm: GV môn nhập trong 3 ngày]
    V --> W[Hệ thống tự động: Tính TB, Gửi PH]
    W --> X[GVCN kiểm tra mỗi tuần]
    
    U --> Y[Nhận xét: GVCN viết cuối HK]
    Y --> Z[Nhập Phần mềm]
    Z --> AA[Trưởng khối kiểm tra: Ngôn ngữ, Chính tả]
    AA --> AB{OK?}
    AB -->|Sai| AC[GVCN sửa]
    AC --> Z
    AB -->|Đúng| AD[Phê duyệt, PH xem được]
    
    L --> AE[Thông báo PH đã cập nhật]
    Q --> AE
    S --> AE
    T --> AE
    AD --> AE
    
    AE --> AF[Ghi log: Ai sửa, Khi nào, Sửa gì]
    AF --> AG[✅ Hoàn thành cập nhật!]
```
