...
```mermaid
graph TD
    A[Tuần 1: Quan sát hàng ngày] --> B[GVCN ghi Phiếu QT06-CL05]
    B --> C[Quan sát: Điểm danh, Tâm trạng, Kết bạn, Ăn uống...]
    C --> D[Xử lý tình huống: Khóc→Ôm, Không ăn→Cho thử...]
    D --> E[Liên hệ PH mỗi ngày khi đón con]
    
    E --> F[Cuối tuần 1: Đánh giá sơ bộ]
    F --> G[Phân loại: A-Tốt, B-TB, C-Khó]
    G --> H[Họp nhóm GV: Thảo luận nhóm C]
    
    H --> I[Tuần 2-3: Hỗ trợ tích cực]
    I --> J[Ghép bạn: HS nhút + HS vui vẻ]
    J --> K[Tăng tự tin: Giao nhiệm vụ nhỏ, Khen ngợi]
    K --> L[Giảm lo lắng: Hỏi thăm, Cho biết lịch]
    L --> M[Liên hệ PH: 1-2 lần/tuần]
    M --> N[Theo dõi nhóm C MỖI NGÀY]
    
    N --> O[Cuối tuần 3: Đánh giá chính thức]
    O --> P[Điền Phiếu QT06-CL06: 10 tiêu chí, 20 điểm]
    P --> Q[Tính tổng điểm]
    
    Q --> R{Điểm?}
    R -->|16-20| S[Thích nghi tốt ✅]
    R -->|10-15| T[Thích nghi TB → Tăng hỗ trợ]
    R -->|0-9| U[Thích nghi khó ⚠️]
    
    S --> V[Tiếp tục theo dõi thường]
    T --> W[Tăng cường hỗ trợ]
    
    U --> X[Tuần 4: Can thiệp đặc biệt]
    X --> Y[Lập danh sách HS < 10 điểm]
    Y --> Z[GVCN gặp riêng HS: Tìm nguyên nhân]
    Z --> AA[Gọi PH đến gặp trực tiếp]
    AA --> AB[Họp: GVCN + PH]
    AB --> AC[Trình bày vấn đề, Đề xuất phương án]
    AC --> AD[Yêu cầu PH hợp tác: Động viên, Đưa đón đúng giờ]
    
    AD --> AE{Cần Tâm lý?}
    AE -->|Lo lắng nặng, Từ chối đến trường| AF[✅ Chuyển Tâm lý]
    AE -->|Lo lắng nhẹ| AG[❌ GVCN xử lý]
    
    AF --> AH[Tâm lý làm việc với HS: 3-5 buổi]
    AH --> AI[Tìm nguyên nhân, Dạy kỹ năng, Giảm lo lắng]
    AI --> AJ[Sau 2 tuần: Họp GVCN+Tâm lý+PH]
    
    AG --> AJ
    W --> AJ
    
    AJ --> AK{Tiến bộ?}
    AK -->|Có| AL[✅ Tiếp tục theo dõi]
    AK -->|Không| AM[Xem xét: Chuyển lớp? Giảm giờ? Nghỉ tạm?]
    
    AM --> AN[Quyết định với PH]
    AL --> AO[Theo dõi liên tục đến HK1]
    V --> AO
    
    AO --> AP[Ghi Sổ liên lạc mỗi tuần]
    AP --> AQ[Họp PH tháng 1 lần]
    AQ --> AR[Báo cáo BP HS tháng 1 lần]
    
    AR --> AS[Cuối HK1: Đánh giá tổng thể]
    AS --> AT[Tỷ lệ thích nghi tốt: ___% Mục tiêu ≥90%]
    AT --> AU[Tỷ lệ bỏ học: ___% Mục tiêu ≤5%]
    AU --> AV[Nộp báo cáo QT06-BC01]
    AV --> AW[✅ Hoàn thành theo dõi thích nghi!]
```
