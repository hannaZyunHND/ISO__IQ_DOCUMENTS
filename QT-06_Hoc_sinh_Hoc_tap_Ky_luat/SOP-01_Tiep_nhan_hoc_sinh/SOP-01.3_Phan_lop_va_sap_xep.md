...
```mermaid
graph TD
    A[Thu thập thông tin HS] --> B[Tạo bảng Excel: Tên, Điểm, Giới, Anh em...]
    B --> C[Phân nhóm: Giỏi/Khá/TB/Yếu]
    
    C --> D[Thuật toán phân đều]
    D --> E[Chia rắn: HS1→A, HS2→B, HS8→H, HS9→H, HS10→G...]
    E --> F[Tính Điểm TB, %Nam từng lớp]
    
    F --> G{Cân bằng?}
    G -->|Chênh > 0.3| H[Điều chỉnh: Swap HS]
    H --> F
    
    G -->|OK| I[Kiểm tra anh/chị em]
    I --> J{Có anh em cùng lớp?}
    J -->|Có| K[Tách ra: Swap với HS khác]
    K --> I
    
    J -->|Không| L[Kiểm tra HS đặc biệt]
    L --> M[Khuyết tật → Lớp tầng 1, GV KN]
    M --> N[Hiếu động → Chia đều, Mỗi lớp ≤2]
    
    N --> O[Xử lý yêu cầu PH]
    O --> P[Học với bạn X? → Xem xét nếu cùng trình độ]
    P --> Q[Học với GV Y? → Từ chối]
    
    Q --> R[BGH phân công GVCN]
    R --> S[Ghép: Lớp nhiều HS yếu → GV kinh nghiệm]
    S --> T[BGH họp duyệt]
    
    T --> U{Duyệt?}
    U -->|Chưa OK| V[Yêu cầu điều chỉnh]
    V --> O
    
    U -->|OK| W[BGH ký duyệt]
    W --> X[Công bố: Web, App, SMS, Bảng tin]
    
    X --> Y[Tiếp nhận phản hồi PH - 1 tuần]
    Y --> Z{Có yêu cầu chuyển lớp?}
    Z -->|Có - Lý do chính đáng| AA[Xem xét, Chuyển nếu hợp lý]
    Z -->|Có - Không chính đáng| AB[Giải thích, Từ chối]
    Z -->|Không| AC[Chốt danh sách cuối cùng]
    
    AA --> AC
    AB --> AC
    AC --> AD[✅ Sẵn sàng khai giảng!]
```
