...
```mermaid
graph TD
    A[PH muốn đăng ký] --> B{Hình thức?}
    B -->|Online 70%| C[Truy cập Website]
    C --> D[Click Đăng ký nhập học]
    D --> E[Điền Form 20 câu]
    E --> F[Upload giấy tờ scan]
    F --> G[Submit]
    G --> H[Nhận mã HS-2024-XXX]
    H --> I[Email/SMS xác nhận tự động]
    
    B -->|Trực tiếp 30%| J[Đến Phòng Tuyển sinh]
    J --> K[NV tiếp đón, Hướng dẫn]
    K --> L[PH điền Form giấy]
    L --> M[PH nộp giấy tờ photo]
    M --> N[NV kiểm tra sơ bộ: Checklist]
    N --> O{Đủ giấy tờ?}
    O -->|Thiếu| P[Hướng dẫn bổ sung]
    P --> Q[PH về bổ sung, Quay lại]
    Q --> M
    O -->|Đủ| R[Nhận hồ sơ, Phát mã]
    R --> S[Phát Phiếu hẹn]
    
    I --> T[3 ngày: Phòng TS xét duyệt]
    S --> T
    T --> U[Kiểm tra: Tuổi? Giấy tờ OK? Còn chỗ?]
    U --> V{Kết quả?}
    
    V -->|Đủ ĐK + Còn chỗ| W[NHẬN]
    W --> X[Gửi Email/SMS thông báo NHẬN]
    X --> Y[Hẹn: Nộp giấy gốc + Đóng phí]
    
    V -->|Đủ ĐK nhưng Hết chỗ| Z[WAITLIST]
    Z --> AA[Xếp vào danh sách chờ]
    AA --> AB[Email thông báo: Vị trí số Y]
    AB --> AC{Có chỗ trống?}
    AC -->|Không| AD[Tiếp tục chờ]
    AC -->|Có| AE[Email: Có chỗ! XN trong 48h]
    AE --> AF{PH xác nhận?}
    AF -->|NHẬN| W
    AF -->|KHÔNG hoặc Không trả lời 48h| AG[Chuyển PH tiếp theo]
    AG --> AE
    
    V -->|Không đủ ĐK| AH[TỪ CHỐI]
    AH --> AI[Email từ chối lịch sự + Gợi ý]
```
