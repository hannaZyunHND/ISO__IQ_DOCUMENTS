...
```mermaid
graph TD
    A[Chuẩn bị: Danh sách HS, Công cụ điểm danh] --> B[7:30 - GV vào lớp]
    B --> C[Chào lớp]
    C --> D[Kiểm tra trang phục nhanh]
    D --> E[Điểm danh]
    
    E --> F{Phương thức?}
    F -->|Giấy| G[Gọi tên từng em, Ghi P/A/L...]
    F -->|Phần mềm| H[Nhìn lớp, Tick trên App]
    F -->|Tự động| I[Xem danh sách đã quẹt thẻ]
    
    G --> J[Thông báo kết quả: Sĩ số, Có mặt, Vắng]
    H --> J
    I --> J
    
    J --> K[Ghi nhận HS vắng]
    K --> L[Gọi điện/Nhắn PH ngay]
    
    L --> M{PH có cho nghỉ?}
    M -->|Có - Có đơn| N[Cập nhật: AP - Vắng có phép]
    M -->|Có - Chưa có đơn| O[Yêu cầu bổ sung đơn trong 3 ngày]
    M -->|Không biết con vắng!| P[⚠️ KHẨN CẤP!]
    
    P --> Q[Báo BP HS, Bảo vệ, BGH]
    Q --> R[Tìm kiếm: Xem camera, Hỏi bạn, Tìm trong trường]
    R --> S{Tìm thấy?}
    S -->|Có| T[Xử lý: Kỷ luật, Gọi PH]
    S -->|Không| U[Gọi 113!]
    ...
    V --> W[Trong ngày: Xử lý tình huống đặc biệt]
    W --> X[HS đến muộn → Ghi L]
    X --> Y[HS xin về sớm → PH ký biên bản, Ghi E]
    Y --> Z[HS ốm → Xuống Y tế, Ghi I hoặc E]
    
    Z --> AA[Cuối ngày: Rà soát, Lưu trữ]
    AA --> AB[Phần mềm: Backup tự động]
    AB --> AC[Giấy: Lưu vào Tủ hồ sơ]
    AC --> AD[✅ Hoàn thành điểm danh ngày]
```
