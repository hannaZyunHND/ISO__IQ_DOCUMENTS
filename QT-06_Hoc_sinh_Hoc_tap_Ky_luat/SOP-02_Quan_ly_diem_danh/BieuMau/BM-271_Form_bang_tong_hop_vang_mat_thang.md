# BM-271: Form bảng tổng hợp vắng mặt tháng

## Thông tin biểu mẫu
- **Mã biểu mẫu**: QT-06-SOP-02-BM-07
- **Tên biểu mẫu**: Form bảng tổng hợp vắng mặt tháng
- **Phiên bản**: 1.0
- **Ngày ban hành**: 30/10/2025
- **Ngày hiệu lực**: 30/10/2025
- **SOP liên quan**: SOP-02.3

## Mục đích
Biểu mẫu để GVCN tổng hợp tình hình vắng mặt của lớp trong tháng và phân loại mức độ.

## Nội dung biểu mẫu

```
═══════════════════════════════════════════════════════
  BẢNG TỔNG HỢP VẮNG MẶT THÁNG __/____
  Lớp: _______  GVCN: _______  Tháng __/____
═══════════════════════════════════════════════════════

I. TỔNG QUAN:
• Sĩ số: ____ HS
• Số ngày học: ____ ngày (= ____ buổi)
• Tỷ lệ đi học trung bình: ___% (So với tháng trước: +/- __%)

II. THỐNG KÊ THEO MỨC ĐỘ:
• 🟢 Bình thường (≥90%): ____ HS (___%)
• 🟡 Chú ý (80-89%):      ____ HS (___%)
• 🟠 Cảnh báo (70-79%):   ____ HS (___%)
• 🔴 Nguy hiểm (<70%):    ____ HS (___%)

III. BẢNG CHI TIẾT:

STT | Họ tên    | Sĩ số | Có  | Vắng| Vắng | Tỷ lệ | Mức độ
    |           | buổi  | mặt | có  | không| đi học|
    |           |       |     | phép| phép |       |
────┼───────────┼───────┼─────┼─────┼──────┼───────┼────────
 1  | Nguyễn VA |  40   |  39 |  1  |   0  | 97.5% | 🟢
 2  | Trần Thị B|  40   |  35 |  5  |   0  | 87.5% | 🟡
 3  | Lê Văn C  |  40   |  30 |  3  |   7  | 75.0% | 🟠 ⚠️
 4  | Phạm Thị D|  40   |  25 |  2  |  13  | 62.5% | 🔴 ⚠️
────┴───────────┴───────┴─────┴─────┴──────┴───────┴────────

IV. PHÂN TÍCH:
• Lý do vắng chính: 
  - Ốm: ___% (So với tháng trước: +/- __%)
  - Việc gia đình: ___%
  - Không phép: ___%
  - Khác: ___%

V. HS ĐÃ CAN THIỆP:
• Lê Văn C: Vắng không phép 7 lần. Đã gọi PH 3 lần.
  Kết quả: PH cam kết cải thiện.

• Phạm Thị D: Vắng không phép 13 lần. Đã họp PH + BGH.
  Kết quả: Xem xét lưu ban.

VI. KẾ HOẠCH THÁNG SAU:
• Theo dõi sát HS mức 🟠🟡
• Gọi PH mỗi tuần
• Họp PH + BGH cho HS mức 🔴

GVCN ký: __________  Ngày: __/__/____
═══════════════════════════════════════════════════════
```

## Hướng dẫn sử dụng
1. Tổng hợp dữ liệu từ các báo cáo tuần
2. Tính tỷ lệ đi học từng HS
3. Phân loại theo 4 mức độ
4. Nộp cho BP HS ngày 5 tháng sau

## Lưu ý
- Phân tích nguyên nhân vắng
- Ghi rõ HS đã can thiệp
- Lập kế hoạch tháng sau
