...
```mermaid
graph TD
    A[Phát hiện vi phạm] --> B{Loại vi phạm?}
    
    B -->|Muộn lần 1-2| C[Nhắc nhở HS]
    B -->|Muộn lần 3-5| D[Ghi Sổ liên lạc, Yêu cầu PH ký]
    B -->|Muộn lần 6+| E[Gọi PH đến gặp]
    E --> F[Lập biên bản, Cam kết cải thiện]
    F --> G{Có cải thiện?}
    G -->|Có| H[✅ Kết thúc]
    G -->|Không| I[Trừ điểm hạnh kiểm]
    
    B -->|Vắng không phép lần 1| J[Gọi PH hỏi lý do]
    J --> K[Yêu cầu bổ sung đơn]
    
    B -->|Vắng không phép lần 2-3| L[Gọi PH đến gặp]
    L --> M[Cảnh báo nghiêm khắc]
    
    B -->|Vắng không phép lần 4+| N[Lập biên bản vi phạm]
    N --> O[Báo BGH]
    O --> P[Hội đồng kỷ luật]
    P --> Q[Quyết định: Cảnh cáo, Đình chỉ, Buộc thôi học]
```
