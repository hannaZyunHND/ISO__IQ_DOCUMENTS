...
```mermaid
graph TD
    A[Sau PV 72h: Có kết quả] --> B[Hội đồng TS ký duyệt QT07-DS02]
    B --> C[Lập 3 danh sách]
    C --> D[Danh sách ĐẬU: 50 HS]
    C --> E[Danh sách DỰ BỊ: 5 HS]
    C --> F[Danh sách KHÔNG ĐẬU: 10 HS]
    
    D --> G[Gọi điện thông báo NGAY]
    E --> G
    F --> G
    
    G --> H{Kết quả?}
    
    H -->|Đậu| I[Chúc mừng! Hẹn hoàn thiện hồ sơ]
    H -->|Dự bị| J[Thông báo: Danh sách chờ]
    H -->|Không đậu| K[Tiếc nuối, Giải thích nhẹ nhàng]
    
    I --> L[Gửi email chính thức: Kết quả + Hướng dẫn]
    J --> L
    K --> L
    
    L --> M[Cập nhật CRM: WON/WAIT/LOST]
    
    M --> N[HS ĐẬU: Chờ hoàn thiện]
    N --> O{PH đến?}
    O -->|Đúng hẹn| P[Hoàn thiện hồ sơ]
    O -->|Không đến - 1 tuần| Q[Gọi nhắc]
    O -->|Không đến - 2 tuần| R[Nhường chỗ HS dự bị]
    
    Q --> O
    
    P --> S[Kiểm tra giấy tờ: Đủ 15 loại?]
    S --> T{Đủ?}
    T -->|Thiếu| U[Yêu cầu bổ sung, Hẹn lại]
    T -->|Đủ| V[Ký hợp đồng QT07-HD01]
    
    V --> W[Đóng phí: Học phí + Ăn + Xe...]
    W --> X[Kế toán cấp Biên nhận QT07-F04]
    X --> Y[Tạo hồ sơ HS: Giấy + Số]
    Y --> Z[Photo/Scan, Nhập Phần mềm]
    Z --> AA[Cấp Thẻ HS, Sổ liên lạc]
    
    AA --> AB[Hẹn Orientation: 01/09]
    AB --> AC[Gửi email xác nhận đã nhập học]
    
    AC --> AD[HS DỰ BỊ: Theo dõi đến 15/04]
    AD --> AE{Có HS từ chối?}
    AE -->|Có| AF[Gọi HS dự bị: Con được nhận!]
    AE -->|Không| AG[Gọi: Xin lỗi, Năm nay hết chỗ]
    
    AF --> P
    AG --> AH[Hoàn phí, Động viên]
    
    K --> AI[Hoàn phí đăng ký: 500K]
    AI --> AJ[Gợi ý trường khác]
    
    AH --> AK[✅ Kết thúc tuyển sinh đợt này!]
    AC --> AK
    AJ --> AK
```
