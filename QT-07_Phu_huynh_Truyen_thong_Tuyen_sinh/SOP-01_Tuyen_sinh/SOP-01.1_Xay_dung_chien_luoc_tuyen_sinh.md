...
```mermaid
graph TD
    A[Tháng 1: Phân tích tình hình] --> B[Phân tích nội bộ: Sức chứa, Nhân lực, Tài chính, Mạnh-Yếu]
    B --> C[Phân tích thị trường: Quy mô, Đối thủ, Xu hướng]
    C --> D[Phân tích năm trước: Đạt? Thiếu? Tại sao?]
    
    D --> E[Tháng 2: Xác định mục tiêu]
    E --> F[Họp BGH: Đặt chỉ tiêu QT07-CT01]
    F --> G[Phân bổ: Theo cấp, Theo tháng, Theo nguồn]
    G --> H[Phân bổ ngân sách QT07-NS01]
    H --> I[80M Facebook, 40M Google, 30M Event...]
    
    I --> J[Xác định đối tượng mục tiêu]
    J --> K[Vẽ 3 Persona: Bà Nguyễn, Ông Trần, Bà Lê]
    K --> L[Mỗi Persona: Tuổi, Nghề, Thu nhập, Quan tâm, Hành vi]
    
    L --> M[Phân tích đối thủ]
    M --> N[Trường X, Y, Z: Mạnh? Yếu? Chiến thuật?]
    N --> O[Vị trí của mình so với đối thủ]
    
    O --> P[Xây dựng chiến lược tổng thể]
    P --> Q[Định vị: Chất lượng quốc tế - Học phí hợp lý]
    Q --> R[USP: Song ngữ, GV chứng chỉ, Cơ sở hiện đại, Giá tốt]
    R --> S[Chiến thuật 4P: Product, Price, Place, Promotion]
    
    S --> T[Lập Chiến lược chi tiết QT07-CL01]
    T --> U[Mục tiêu, Chiến thuật, Timeline, KPI]
    U --> V[Trình BGH]
    ...
    W -->|Có| Y[BGH ký duyệt]
    Y --> Z[Triển khai từ Tháng 3]
    Z --> AA[✅ Bắt đầu tuyển sinh!]
```
