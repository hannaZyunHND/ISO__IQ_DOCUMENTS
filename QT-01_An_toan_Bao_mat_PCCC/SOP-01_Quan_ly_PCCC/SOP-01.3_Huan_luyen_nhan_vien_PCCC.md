...
```mermaid
graph TD
    A[Bắt đầu năm học mới] --> B[Đánh giá nhu cầu đào tạo]
    B --> C[Lập kế hoạch đào tạo]
    C --> D[Trình Ban Giám hiệu]
    D --> E{Phê duyệt?}
    
    E -->|Không| F[Điều chỉnh kế hoạch]
    F --> D
    
    E -->|Có| G[Phân bổ ngân sách]
    G --> H[Liên hệ giảng viên]
    H --> I[Chuẩn bị tài liệu, thiết bị]
    I --> J[Thông báo lịch đào tạo]
    
    J --> K[Đào tạo lý thuyết]
    K --> L[Kiến thức cơ bản về cháy]
    L --> M[Nguyên nhân và phòng ngừa]
    M --> N[Hệ thống PCCC trường học]
    N --> O[Kế hoạch ứng phó]
    
    O --> P[Đào tạo thực hành]
    P --> Q[Sử dụng bình chữa cháy]
    Q --> R[Thoát hiểm qua khói]
    R --> S[Sơ tán có tổ chức]
    S --> T[Sơ cứu ban đầu]
    
    T --> U[Kiểm tra đánh giá]
    U --> V[Trắc nghiệm lý thuyết]
    V --> W[Thực hành bình chữa cháy]
    W --> X[Thực hành sơ tán]
    
    X --> Y{Điểm tổng >= 70%?}
    Y -->|Không| Z[Học lại]
    Z --> K
    
    Y -->|Có| AA[Cấp chứng chỉ]
    AA --> AB[Lưu hồ sơ đào tạo]
    AB --> AC[Thu thập phản hồi]
    AC --> AD[Lập báo cáo]
    AD --> AE[Đánh giá và cải tiến]
    
    AE --> AF[Nhắc nhở định kỳ]
    AF --> AG[Kết thúc]
```
