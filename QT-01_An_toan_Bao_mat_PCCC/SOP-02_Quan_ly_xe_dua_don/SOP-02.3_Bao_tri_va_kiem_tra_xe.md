...
```mermaid
graph TD
    A[Mỗi sáng trước ca] --> B[Tài xế kiểm tra xe]
    B --> C[Kiểm tra bên ngoài]
    C --> D[Kiểm tra bên trong]
    D --> E[Kiểm tra động cơ]
    
    E --> F{Tất cả OK?}
    F -->|Có| G[Ghi Sổ: OK]
    F -->|Lỗi nhỏ| H[Ghi nhận, sửa sau]
    F -->|Lỗi nặng| I[Báo ngay, không đi]
    
    G --> J[Xuất bến]
    H --> J
    I --> K[Dùng xe dự phòng]
    
    J --> L[Vận hành hàng ngày]
    L --> M{Cuối tuần}
    
    M --> N[Vệ sinh xe toàn bộ]
    N --> O[Kiểm tra cơ bản]
    O --> P[Bơm lốp, thêm dầu]
    
    P --> Q{Cuối tháng}
    Q --> R[Kiểm tra chuyên sâu]
    R --> S[Lập danh sách cần sửa]
    S --> T[Trình phê duyệt]
    T --> U[Thực hiện sửa chữa]
    
    U --> V{6 tháng/lần}
    V --> W[Đưa garage bảo dưỡng]
    W --> X[Thay dầu, lọc]
    X --> Y[Kiểm tra toàn bộ]
    Y --> Z[Nghiệm thu]
    
    Z --> AA{1 năm/lần}
    AA --> AB[Chuẩn bị hồ sơ đăng kiểm]
    AB --> AC[Đưa xe đi đăng kiểm]
    AC --> AD[Nhận tem đăng kiểm]
    AD --> AE[Cập nhật hồ sơ xe]
    AE --> AF[Kết thúc chu kỳ]
```
