...
```mermaid
graph TD
    A[Phát sinh nhu cầu điều chỉnh NS] --> B[BP/BGH lập đề án]
    B --> C[Giải trình lý do, số tiền]
    C --> D[Đề xuất nguồn NS]
    D --> E[Phòng KT thẩm định]
    E --> F[Trình BGH]
    
    F --> G{Mức độ?}
    G -->|< 5% hoặc < 100tr| H[BGH quyết định]
    G -->|>= 5% hoặc >= 100tr| I[Trình HĐ quản trị]
    
    H --> J{Duyệt?}
    I --> J
    
    J -->|Không| K[Từ chối, giải thích]
    J -->|Có| L[Ký phê duyệt]
    
    L --> M[Thông báo BP liên quan]
    M --> N[Cập nhật hệ thống]
    N --> O[Lưu Biên bản điều chỉnh]
    O --> P[Theo dõi sử dụng NS mới]
    P --> Q[Đánh giá hiệu quả]
```
