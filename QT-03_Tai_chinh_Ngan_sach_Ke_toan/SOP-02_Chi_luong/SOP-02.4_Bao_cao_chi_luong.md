...
```mermaid
graph TD
    A[Sau ngày 5: Chi lương xong] --> B[Thu thập dữ liệu]
    B --> C[So sánh tháng trước]
    C --> D[So sánh ngân sách]
    D --> E[Lập BC chi lương tháng]
    
    E --> F[Phân tích A: Tổng quan]
    E --> G[Phân tích B: Theo bộ phận]
    E --> H[Phân tích C: Cấu trúc lương]
    E --> I[Phân tích D: Khấu trừ]
    
    F --> J[Viết nhận xét, dự báo]
    G --> J
    H --> J
    I --> J
    
    J --> K[Ngày 7-10: Trình BGH]
    K --> L{BGH có câu hỏi?}
    L -->|Có| M[Giải trình chi tiết]
    L -->|Không| N[Lưu hồ sơ]
    
    O[Cuối quý] --> P[Lập BC quý]
    P --> Q[Trình BGH + HĐ]
    
    R[Cuối năm] --> S[Quyết toán lương năm]
    S --> T[Dùng kê khai thuế]
    T --> U[Lưu trữ 10 năm]
```
