...
```mermaid
graph TD
    A[PH quyết định cho con nghỉ học] --> B[Nộp đơn xin hoàn phí]
    B --> C[Phòng KT tiếp nhận]
    C --> D[Kiểm tra hồ sơ]
    D --> E[Rà soát tình trạng đóng học phí]
    E --> F[Kiểm tra tài sản mượn]
    
    F --> G{Còn nợ/mất tài sản?}
    G -->|Có| H[Yêu cầu thanh toán trước]
    H --> I[PH thanh toán]
    I --> J[Tính số tiền hoàn]
    
    G -->|Không| J
    J --> K[Lập phiếu đề xuất]
    K --> L[Trình Kế toán trưởng]
    L --> M[Trình BGH]
    
    M --> N{BGH duyệt?}
    N -->|Không| O[Giải thích lý do cho PH]
    N -->|Có| P[Thông báo PH]
    
    P --> Q[Trong 30 ngày: Chuyển khoản]
    Q --> R[PH ký xác nhận nhận tiền]
    R --> S[Hạch toán giảm doanh thu]
    S --> T[Lưu hồ sơ]
```
